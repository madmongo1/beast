...
template<class GenTx, class GenRx>
void test_v2_operation(GenTx gen_tx, GenRx gen_rx)
{
    net::io_context ioc;
    auto remote = test::stream(ioc);
    auto tx = gen_tx();

    auto local = connect(remote);
    auto rx = gen_rx();

    const auto expected = std::string("the quick brown fox jumps over the lazy dog.");
    auto ftx = std::async(std::launch::async, [&]{
        auto sent = 0;
        while(sent < 1000)
        {
            int chunk = 5;
            while (chunk && sent < 1000 && tx.size() + expected.size() <= tx.max_size())
            {
                auto in = tx.size();
                const auto grow = 100;
                tx.grow(grow);
                tx.shrink(grow - net::buffer_copy(tx.data(in, grow), net::buffer(expected)));
                ++sent;
                --chunk;
            }
            net::write(remote, tx);
    ...
    while(!ec)
    {
        auto rxlen = net::read_until(local, rx, '.', ec);
        BEAST_EXPECTS(!ec, ec.message());
        BEAST_EXPECT(rxlen == expected.size());
        BEAST_EXPECT(rx.size() >= rxlen);
        auto result = buffers_to_string(rx.data(0, rxlen));
        BEAST_EXPECTS(result == expected, result);
        rx.consume(rxlen);
        if(++received >= 1000)
            break;
